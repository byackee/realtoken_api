...
# ----------------------------
NODE_SCRIPT_WALLETBALANCE = "/app/fetch_walletbalance_gnosis.js"
...
async def ensure_wallet_exists_and_sync(wallet_address: str, script_type: str = "wallet"):
    ...
    try:
        ...
        if not wallet_exists:
            ...
            last_sync = await get_last_execution_time(sync_request_id)
            if last_sync is not None:
                # S'assurer que last_sync a un timezone
                ...
                if elapsed < MIN_INTERVAL:
                    ...
                    print(f"⏱️ Synchronisation trop récente, veuillez patienter encore {remaining_time:.2f} secondes")
                    return {"status": "pending", "message": f"Synchronisation trop récente, réessayer dans {remaining_time:.2f} secondes"}
            print("🔄 Synchronisation du wallet existant")
            if script_type == "wallet":
                await run_node_script_async(NODE_SCRIPT_WALLETBALANCE)
                await run_node_script_async(NODE_SCRIPT_RMMBALANCE)
            elif script_type == "yam":
                ...
                await run_node_script_async(NODE_SCRIPT_TRANSACTIONS_HISTORY, wallet_address)
            await update_execution_time(sync_request_id)
            return {"status": "success", "message": f"Wallet synchronisé"}
